...
/* Faithful JSX port of lod.md (Particle Tether / OrbConverge):
   a draggable orb of converging particles on 2D canvas. No dependencies. */
...
function __OriginkitBase_OrbConverge(props) {
  const {
    style,
    dotColor = '#b8a6ff',
    density = 300,
  ...
  useEffect(() => {
    ...
    const render = (now) => {
      ...
      const accCol = css(parseColor(v.acc, [244, 241, 234, 1]));
...
export default function OrbConverge(props) {
  return <__OriginkitBase_OrbConverge {...PRESET} {...props} />;
}
